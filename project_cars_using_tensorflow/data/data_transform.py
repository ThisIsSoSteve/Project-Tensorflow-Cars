...
def raw_to_training_data(raw_save_path, training_save_path):
    ...
    for filename in tqdm(listing):
        ...
        image = cv2.imread(filename + '-image.png', cv2.IMREAD_GRAYSCALE)
        image = cv2.resize(image, (128,72)) #16:9 ratio
        gray_image = np.float16(image / 255.0) #0-255 to 0.0-1.0

        label = np.array([throttle / 255, brakes / 255, steering_left / 32768, steering_right / 32767]) #throttle, brakes, left, right
