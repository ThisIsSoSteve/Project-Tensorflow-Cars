...
def init_batches(training_file_path):
     #load data
    training_data = np.load(training_file_path + '/training.npy')
    np.random.shuffle(training_data)
    percent_of_test_data = 200#int((len(training_data) / 100) * 10) # 10 percent
...
def train_model(number_of_epochs, batch_size, learning_rate, training_file_path, checkpoint_file_path):
    ...
    training_data, test_data = init_batches(training_file_path)
    ...
    with tf.Session() as sess:#config=config
        with tf.device('/gpu:0'):
            ...
            while sess.run(global_step) < number_of_epochs:
                ...
                print('Global Step', sess.run(global_step), 'Loss', epoch_loss, 'Accuracy %g' % accuracy.eval(feed_dict={model.x: test_x, model.y: test_y, model.p_keep_hidden: 1.0}))
