...
#     prediction = model.myModel(model.x, model.p_keep_hidden, model.batch_size)
...
#                 _, loss_val = sess.run([optimizer, cost], feed_dict = {model.x: batch_x, model.y: batch_y, model.p_keep_hidden: 0.9, model.batch_size: abs(batch_starting_index - batch_ending_index)})
...
#                     current_accuracys.append(accuracy.eval(feed_dict={model.x: validation_batch_x, model.y: validation_batch_y, model.p_keep_hidden: 1.0, model.batch_size: abs(batch_starting_index - batch_ending_index)}))
        ...
        with tf.Session() as sess:#config=config
            ...
            while step < number_of_epochs:
                ...
                while starting_batch_index < number_of_training_records:
                    ...
                    _, loss_val = sess.run(self.model.optimize, {self.X: batch_x, self.Y: batch_y})
                ...
                if(step % 10 == 0):
                    ...
                    while starting_batch_index < number_of_validation_records:
                        ...
                        current_accuracy = sess.run(self.model.error, { self.X: validation_batch_x, self.Y: validation_batch_y })
